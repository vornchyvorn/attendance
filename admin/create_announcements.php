...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($title && $content) {
        ...
        $stmt->bind_param("ssis", $title, $content, $created_by, $status);

        if ($stmt->execute()) {
            $success = "✅ សេចក្ដីជូនដំណឹងបានបញ្ចូលដោយជោគជ័យ!";
        } else {
            $error = "❌ ការបញ្ចូលបរាជ័យ!";
        }

        $stmt->close();
    } else {
        $error = "សូមបំពេញទិន្នន័យអោយគ្រប់គ្រាន់!";
    }
}
?>
...
<main class="flex-1 ml-0 transition-all duration-300" :class="sidebarOpen ? 'ml-64' : 'ml-0'">
  ...
  <div class="max-w-xl mx-auto bg-white p-6 rounded shadow">
    ...
    <form method="POST" class="space-y-4">
      ...
      <div>
        <label class="block mb-1 font-semibold">មាតិកា</label>
        <textarea name="content" rows="5" class="w-full border px-3 py-2 rounded" required></textarea>
      </div>

      <div>
        <label class="block mb-1 font-semibold">ស្ថានភាព</label>
      ...
      <div class="text-right">
        <button type="submit" class="bg-sky-500 text-white px-4 py-2 rounded hover:bg-sky-600">បញ្ចូល</button>
      </div>
    </form>
  </div>
