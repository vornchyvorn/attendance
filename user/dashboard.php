...
<main class="flex-grow">
   ...
   <?php if ($showMorning): ?>
    <div class="border-2 border-blue-400 bg-teal-600 p-6 rounded-lg w-full max-w-xs mx-auto mb-4">
      ...
      <form method="post" action="checkin.php" class="inline-block">
        ...
        <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-lg shadow hover:bg-blue-600">ចូល</button>
      </form>
      <form method="post" action="checkout.php" class="inline-block mt-2">
        <input type="hidden" name="time_period" value="morning">
        <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-lg shadow hover:bg-blue-600">ចេញ</button>
      </form>
    </div>
  <?php endif; ?>
     <?php if ($showEvening): ?>
  ...
  <?php endif; ?>
    <h3 class="text-xl font-bold mb-4 text-center text-sky-700 underline hover:text-teal-700 duration-100 "> ប្រវត្តិនៃការចូលរួម</h3>
          ...
          <i class="fa-solid fa-magnifying-glass mr-1"></i>
            ...
            <div class="text-start">
              <select name="limit" onchange="this.form.submit()"
                ...
                <option value="5" <?= ($_GET['limit'] ?? '') == 5 ? 'selected' : '' ?>>5</option>
...
<div class="flex flex-wrap justify-center mt-6 gap-2 px-4">
  <?php
    $url = "?";
    if (!empty($_GET['filter_date'])) $url .= "filter_date=" . $_GET['filter_date'] . "&";
    if (!empty($_GET['limit'])) $url .= "limit=" . $_GET['limit'] . "&";
